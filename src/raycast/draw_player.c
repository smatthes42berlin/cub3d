...
void	draw_player_direction(t_main_data *data, int *addr)
{
	...
	scaled_line_length = 30 * data->w.scale_factor;
	start_x = (data->player.x + data->player.size / 2) * data->w.scale_factor;
	start_y = (data->player.y + data->player.size / 2) * data->w.scale_factor;
...
void	draw_player(t_main_data *data, int *addr)
{
	...
	y = 0;
	scaled_size = data->player.size * data->w.scale_factor;
	while (y < scaled_size)
	{
		x = 0;
		while (x < scaled_size)
		{
			scaled_x = data->player.x * data->w.scale_factor + x;
			scaled_y = data->player.y * data->w.scale_factor + y;
